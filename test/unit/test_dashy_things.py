...
def test_valid_zexcoin_address():
    from zexcoinlib import is_valid_zexcoin_address

    main = valid_zexcoin_address()
    test = valid_zexcoin_address('testnet')

    assert is_valid_zexcoin_address(main) is True
    assert is_valid_zexcoin_address(main, 'mainnet') is True
    assert is_valid_zexcoin_address(main, 'testnet') is False

    assert is_valid_zexcoin_address(test) is False
    assert is_valid_zexcoin_address(test, 'mainnet') is False
    assert is_valid_zexcoin_address(test, 'testnet') is True


def test_invalid_zexcoin_address():
    from zexcoinlib import is_valid_zexcoin_address

    main = invalid_zexcoin_address()
    test = invalid_zexcoin_address('testnet')

    assert is_valid_zexcoin_address(main) is False
    assert is_valid_zexcoin_address(main, 'mainnet') is False
    assert is_valid_zexcoin_address(main, 'testnet') is False

    assert is_valid_zexcoin_address(test) is False
    assert is_valid_zexcoin_address(test, 'mainnet') is False
    assert is_valid_zexcoin_address(test, 'testnet') is False
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from zexcoinlib import elect_mn
...
def test_parse_masternode_status_vin():
    from zexcoinlib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % zexcoinlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(zexcoinlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(zexcoinlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(zexcoinlib.blocks_to_seconds(16616)) == 2612035
