...
def has_zexcoin_conf():
    import config
    import io

    valid_zexcoin_conf = False

    # ensure zexcoin_conf exists & readable
    #
    # if not, print a message stating that zexcoin Core must be installed and
    # configured, including JSONRPC access in zexcoin.conf
    try:
        f = io.open(config.zexcoin_conf)
        valid_zexcoin_conf = True
    except IOError as e:
        print(e)

    return valid_zexcoin_conf
...
def main():
    ...
    if not has_zexcoin_conf():
        print("zexcoinCore must be installed and configured, including JSONRPC access in zexcoin.conf")
        sys.exit(1)
