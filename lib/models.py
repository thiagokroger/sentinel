...
class GovernanceObject(BaseModel):
    ...
    # sync zexcoind gobject list with our local relational DB backend
    @classmethod
    def sync(self, zexcoind):
        golist = zexcoind.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_zexcoind(zexcoind, item)
    ...
    @classmethod
    def import_gobject_from_zexcoind(self, zexcoind, rec):
        import decimal
        import zexcoinlib
        import inflection
        ...
        # shim/zexcoind conversion
        object_hex = zexcoinlib.SHIM_deserialise_from_zexcoind(object_hex)
        objects = zexcoinlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from zexcoind...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from zexcoind, with every run
        ...
        # get/create, then sync payment amounts, etc. from zexcoind - zexcoind is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(zexcoind)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from zexcoind! %s" % e)
            govobj.vote_delete(zexcoind)
            return (govobj, None)
    ...
    def vote_delete(self, zexcoind):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(zexcoind, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, zexcoind, signal, outcome):
        import zexcoinlib
        ...
        output = zexcoind.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = zexcoinlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(zexcoind, signal)

    def sync_network_vote(self, zexcoind, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = zexcoind.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = zexcoinD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 zexcoin addr, non-multisig
            if not zexcoinlib.is_valid_zexcoin_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid zexcoin address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (zexcoinlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/zexcoinDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import zexcoinlib
        obj_data = zexcoinlib.SHIM_serialise_for_zexcoind(self.serialise())
    ...
    def prepare(self, zexcoind):
        try:
            object_hash = zexcoind.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = zexcoinD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not zexcoinlib.is_valid_zexcoin_address(addr, config.network):
    ...
    def hash(self):
        import zexcoinlib
        return zexcoinlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = zexcoinD_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, zexcoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - zexcoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, zexcoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - zexcoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, zexcoind):
        now = int(time.time())
        return (self.created_at < (now - zexcoind.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, zexcoind):
        if self.is_expired(zexcoind):
            return False

        return True

    def is_deletable(self, zexcoind):
        if self.is_expired(zexcoind):
            return True
