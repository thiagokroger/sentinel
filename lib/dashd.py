...
class zexcoinDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some zexcoind methods
    ...
    @classmethod
    def from_zexcoin_conf(self, zexcoin_dot_conf):
        from zexcoin_config import zexcoinConfig
        config_text = zexcoinConfig.slurp_config_file(zexcoin_dot_conf)
        creds = zexcoinConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = zexcoinlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = zexcoinlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = zexcoinlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_zexcoind_proto_version_with_sentinel_ping)
