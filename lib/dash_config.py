...
class zexcoinConfig():
    ...
    def slurp_config_file(self, filename):
        # read zexcoin.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
            if re.match('^\s*#', line):
        ...
        # data is zexcoin.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from zexcoin.conf
        match = re.findall(r'rpc(user|password|port)=(.*?)$', data, re.MULTILINE)

        # python >= 2.7
        creds = {key: value for (key, value) in match}

        # standard zexcoin defaults...
        default_port = 9998 if (network == 'mainnet') else 19998

        # use default port for network if not specified in zexcoin.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from zexcoin.conf
