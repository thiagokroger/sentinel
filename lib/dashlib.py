...
def is_valid_zexcoin_address(address, network='mainnet'):
    ...
    zexcoin_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid zexcoin address
        return False

    if (address_version != zexcoin_version):
        return False
...
# shims 'til we can fix the zexcoind side
def SHIM_serialise_for_zexcoind(sentinel_hex):
    from models import zexcoinD_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = zexcoinD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in zexcoind
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # zexcoind expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    zexcoind_hex = serialise(obj)
    return zexcoind_hex


# shims 'til we can fix the zexcoind side
def SHIM_deserialise_from_zexcoind(zexcoind_hex):
    from models import zexcoinD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(zexcoind_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('zexcoin.conf')
